...
<body>
	...
	<div class="text-center">
		<h1 class="display-1">${instructor.firstName} ${instructor.lastName}</h1>
		<h1 class="header-4">${instructor.email}</h1>
		<c:url var="addCourse" value="/instructor/listCourse">
	...
	<table class="table table-hover table-dark table-striped">
		...
		<c:forEach var="tempInstructor" items="${instructor.courses}">
			...
			<c:url var="removeLink" value="/instructor/removeCourseToInstructor">
				<c:param name="courseId" value="${tempInstructor.id}"></c:param>
				<c:param name="instructorId" value="${instructor.id}"></c:param>
			</c:url>
			
			<tr>
				<td>${tempInstructor.courseCode}</td>
				<td>${tempInstructor.courseTitle}</td>
				<td>
						
					<a href="${removeLink}"
						class="btn btn-danger btn-sm"
						onclick="if (!(confirm('Are you aure you want to delete this Course from ${instructor.firstName}?'))) return false">Delete</a>
