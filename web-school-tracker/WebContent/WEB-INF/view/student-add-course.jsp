...
<body>
	...
	<table class="table table-hover table-dark table-striped">
		...
		<c:forEach var="tempCourse" items="${course}">
			...
			<c:url var="addLink" value="/student/addCourseToStudent">
			...
			<c:forEach var="tempStudentCourse" items="${student.courses}">
				<c:if test="${tempStudentCourse.id == tempCourse.id}">
					<c:set var="contains" value="true"></c:set>
				</c:if>
			</c:forEach>
			<c:if test="${contain == false}">
			<tr>
